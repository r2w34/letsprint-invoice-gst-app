...
export default function ProductIndexTable() {
    ...
    try {
     const response = await window.authenticatedFetch(url, {
      ...
      if (data.products.length > 0) {
       ...
       const flattenedProducts = allFetchedProducts.map((product) => ({
         id: product.id,
         title: product.title,
 ...
 //   setIsLoading(true);
  //   try {
 //     const response = await window.authenticatedFetch("/api/products/getProducts", {
 //       method: "GET",
 //       headers: { "Content-Type": "application/json" },
 //     });
  //     const data = await response.json();
 ...
 const syncProducts = async () => {
   ...
   try {
     const response = await window.authenticatedFetch("/api/add-store-products", {
       method: "POST",
       headers: { "Content-Type": "application/json" },
       body: JSON.stringify({
         storeDomain: storeDomain,
         email: email,
         products: allProducts.map(({ id, title, editableHSN, editableGST }) => ({
           productId: id,
           productName: title,
           hsn: editableHSN,
           gst: editableGST,
...
const handleBulkAction = () => {
 ...
 const saveChanges = async () => {
   ...
   try {
     const response = await window.authenticatedFetch("/api/products/update", {
       method: "POST",
       headers: {
