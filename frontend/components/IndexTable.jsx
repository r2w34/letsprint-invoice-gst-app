 ...
 const [isAppTaxApplied, setIsAppTaxApplied] = useState(false);
  ...
  useEffect(() => {
    const fetchStoreDetails = async () => {
      try {
        const response = await window.authenticatedFetch("/api/2024-10/shop.json", {
          method: "GET",
        ...
        if (data.data?.data?.length > 0) {
          ...
          try {
            const billingResponse = await window.authenticatedFetch(
  ...
  useEffect(() => {
    if (storeDomain) {
      window.authenticatedFetch(`/api/get-invoice-template?storeDomain=${storeDomain}`)
  ...
  //     const response = await window.authenticatedFetch("/api/send-invoice", {
  //       method: "POST",
  //       body: formData,
  //     });
  ...
  useEffect(() => {
    if (shopId) {
      window.authenticatedFetch(`/api/smtp/get?shopId=${shopId}`, {
        method: "GET",
        headers: { "Content-Type": "application/json" },
      })
        .then((response) => response.json())
  ...
  //     try {
  //       const response = await window.authenticatedFetch(url, { mode: "cors" });
  ...
  const fetchInvoiceSettings = async () => {
    ...
    return window.authenticatedFetch("/api/fetch-invoice-settings", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
